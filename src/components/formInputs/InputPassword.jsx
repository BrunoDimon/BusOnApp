...
export const InputPassword = ({ label, erro, dica, inputValue, inputOnChange, isDisabled, isInvalid, isReadOnly, isRequired, keyboardType, enterKeyHint }) => {
    ...
    return (
        <FormInput label={label} erro={erro} dica={dica} isDisabled={isDisabled} isInvalid={isInvalid} isReadOnly={isReadOnly} isRequired={isRequired}>
            <Input h={50} borderRadius={'$xl'} $focus-borderColor={'$trueGray400'}>
                <InputField type={showPassword ? 'text' : 'password'} keyboardType={keyboardType} value={inputValue?.toString()} autoCapitalize="none" onChangeText={(v) => inputOnChange(v)} enterKeyHint={enterKeyHint} />
