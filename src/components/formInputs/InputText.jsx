...
export const InputText = ({ label, erro, dica, inputValue, inputOnChange, isDisabled, isInvalid, isReadOnly, isRequired, keyboardType, autoCapitalize, enterKeyHint }) => {
    return (
        <FormInput label={label} erro={erro} dica={dica} isDisabled={isDisabled} isInvalid={isInvalid} isReadOnly={isReadOnly} isRequired={isRequired}>
            <Input h={50} borderRadius={'$xl'} $focus-borderColor={'$trueGray400'}  >
                <InputField type={'text'} keyboardType={keyboardType} autoCapitalize={autoCapitalize} value={inputValue?.toString()} onChangeText={(v) => inputOnChange(v)} enterKeyHint={enterKeyHint} />
            </Input>
        </FormInput>
